...
def ControlChannel():
    ...
    while True:
        time.sleep(2)
        try:
            CH = csmapi2.pull(MAC,'__Ctl_O__', NewSession)
            if CH != []:
                ...
                if cmd == 'RESUME':  
                    ...
                    csmapi2.push(MAC,'__Ctl_I__',['SET_DF_STATUS_RSP',{'cmd_params':CH[0][1][1]['cmd_params']}], NewSession)
                    DF_STATUS = list(CH[0][1][1]['cmd_params'][0])
                    SelectedDF = []
                    index=0            
                    profile['df_list'] = csmapi2.pull(MAC, 'profile')['df_list']              #new
...
def detect_local_ec():
    ...
    while EASYCONNECT_HOST==None:
        ...
        if str(data.decode()) == 'easyconnect':
            ...
            csmapi2.ENDPOINT=EASYCONNECT_HOST
            #print('IoTtalk server = {}'.format(csmapi2.ENDPOINT))
...
def register_device(addr):
    ...
    if csmapi2.ENDPOINT == None: detect_local_ec()
    ...
    print('IoTtalk Server = {}'.format(csmapi2.ENDPOINT))
    profile['d_name'] = csmapi2.register(MAC,profile)
...
def pull(FEATURE_NAME):
    ...
    if state == 'RESUME': data = csmapi2.pull(MAC,FEATURE_NAME)
    else: data = []
...
def push(FEATURE_NAME, *data):
    if state == 'RESUME':
        return csmapi2.push(MAC, FEATURE_NAME, list(data))
    else: return None

def get_alias(FEATURE_NAME):
    try:
        alias = csmapi2.get_alias(MAC,FEATURE_NAME)
    except Exception as e:
        #print (e)
        return None
    else:
...
def set_alias(FEATURE_NAME, alias):
    try:
        alias = csmapi2.set_alias(MAC, FEATURE_NAME, alias)
    except Exception as e:
        #print (e)
        return None
    else:
